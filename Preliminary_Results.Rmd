...
Scenario.Full.Den = Data.Denmark.distribution %>% 
  filter(Period == 2) %>%
  select(age_grp, type, Sexe, freq) %>%
  group_by(age_grp, type, Sexe, freq) %>%
  slice(rep(1:n(), each = 42))%>%
  ...
  merge(negaWatt.data,
        by.x = c("Year", "type"),
        by.y = c("Year", "Type")) %>%
  mutate(Adjusted.distri = freq * value)
...
All.data.DenI = Pop.proj %>% 
  ...
  mutate(Type = ifelse(Type == "Total_Cycling", "cycling", "walking"),
         ...
         Pop.const = first(Pop),
         MR.const = first(Mortality.rate),
         HEAT.nw = HEAT_country(type = Type,
             volume = volume,
             Population = Pop,
             Mortality_Rate = Mortality.rate),
         HEAT.bsl = HEAT_country(type = Type,
             volume = Volume.const,
             Population = Pop,
             Mortality_Rate = Mortality.rate),
         HEAT = -HEAT.nw - (-HEAT.bsl))
...
All.data.Full.Den %>% 
  filter(type == "walking") %>% 
  ggplot() + geom_point(aes(x = year, y = volume, group = age_grp, color = age_grp))

All.data.Full.Den %>% 
  ggplot() + geom_point(aes(x = year, y = Pop, group = age_grp, color = age_grp))
...
#Constant variables
All.data.DenI = All.data.DenI %>% 
  group_by(Type, sexe, age_grp) %>% 
  mutate(Volume.const = first(volume),
         Pop.const = first(Pop),
         MR.const = first(Mortality.rate))

# Effect of pop growth
All.data.DenI %>% 
  mutate( HEAT_pop = HEAT_country(type = Type,
             volume = Volume.const,
             Population = Pop,
             Mortality_Rate = MR.const)) %>% 
  group_by(Type, year) %>% 
  summarise(HEAT = sum(-HEAT_pop)) %>% 
  ggplot() + geom_line(aes(x = year, y = HEAT, group = Type , color = Type))+
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Effect of demographic change")

# Effect of mortality rate evolution
All.data.DenI %>% 
  mutate( HEAT_MR = HEAT_country(type = Type,
             volume = Volume.const,
             Population = Pop.const,
             Mortality_Rate = Mortality.rate)) %>% 
  group_by(Type, year) %>% 
  summarise(HEAT = sum(-HEAT_MR)) %>% 
  ggplot() + geom_line(aes(x = year, y = HEAT, group = Type , color = Type))+
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Effect of mortality rate evolution")

# Effect of increase in volume
All.data.DenI %>% 
  mutate( HEAT_volume = HEAT_country(type = Type,
             volume = volume,
             Population = Pop.const,
             Mortality_Rate = MR.const)) %>% 
  group_by(Type, year) %>% 
  summarise(HEAT = sum(-HEAT_volume)) %>% 
  ggplot() + geom_line(aes(x = year, y = HEAT, group = Type , color = Type))+
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Effect of increase in volume")
...
Graph_TotalperYear_Full.Den = All.data.Full.Den %>% 
  group_by(year, type) %>% 
  summarise(HEAT = sum(HEAT)) %>% 
  ggplot() + geom_bar(aes(x= year, y= HEAT, group = type, fill = type), stat = "identity")+
   ylab("premature deaths avoided") +
...
Graph_TotalperYear_Full.Den

ggsave("Graph_TotalperYear_Full.Den.png", plot = Graph_TotalperYear_Full.Den)
...
All.data.Full.Den.table = All.data.Full.Den %>% 
  group_by(sexe, type, year, age_grp) %>%
  mutate(HEAT = - HEAT) %>% 
  summarise(HEAT = sum(HEAT)) %>% 
  pivot_wider( names_from = year, values_from = HEAT)%>% 
...
Graph_YearlyAvg_Full.Den =  ggplot() + 
  geom_line(data = subset(All.data.Full.Den.table, type %in% c("cycling") & sexe %in% c("Male")),
            aes(x = age_grp,y = Avr_per_year,group = sexe,color = "Den I - Cycling - Male")) +
  geom_line(data = subset(All.data.Full.Den.table, type %in% c("cycling") & sexe %in% c("Female")),
            aes(x = age_grp,y = Avr_per_year,group = sexe,color = "Den I - Cycling - Female")) +
  geom_line(data = subset(All.data.Full.Den.table, type %in% c("walking") & sexe %in% c("Male")),
            aes(x = age_grp,y = Avr_per_year,group = sexe,color = "Den I - Walking - Male")) +
  geom_line(data = subset(All.data.Full.Den.table, type %in% c("walking") & sexe %in% c("Female")),
            aes(x = age_grp, y = Avr_per_year, group = sexe, color = "Den I - Walking - Female")) +
  labs(color = "") +
  ylab("Yearly average of premature deaths avoided") +
  xlab("Age") +
